# UI Router Refactoring

**Category:** Architecture / Technical Debt  
**Status:** 🔄 Postponed  
**Last Updated:** 2025-12-30  
**Related:** [Service Reorganization](../03-architecture-planning/service-reorganization.md), [Clean Architecture](../02-architecture/core-philosophy.md)
...
- ❌ Refactoring in isolation causes duplicate work when redesigning UI
- ❌ Services may not align with new UI architecture
- ❌ Template changes would require service updates
...
- ✅ **Better architecture** - Services designed for actual UI needs
...
## Planned Service: UIRenderingService
...
### Priority 4: Miscellaneous (3 routes)
...
`ui.py` refactoring will follow same patterns as Phase 1:
...
```python
# BEFORE (ui.py - direct Model access)
@router.get("/library/artists")
async def list_artists(session: AsyncSession):
    stmt = select(ArtistModel).options(...)
    result = await session.execute(stmt)
    artists = result.scalars().all()
    return templates.render("artists.html", artists=artists)

# AFTER (with UIRenderingService)
@router.get("/library/artists")
async def list_artists(
    rendering_service: UIRenderingService,
    limit: int = 25,
    offset: int = 0
):
    result = await rendering_service.get_artists_for_browser(
        limit=limit,
        offset=offset
    )
    return templates.render("artists.html", result=result)
```

## Integration with Web UI Design
...
- [Service Reorganization](../03-architecture-planning/service-reorganization.md) - Overall refactoring plan
- [Clean Architecture](../02-architecture/core-philosophy.md) - Architectural principles
- [HTMX Patterns](../05-developer-guides/htmx-patterns.md) - UI interaction patterns
- [Component Library](./component-library.md) - UI component reference
