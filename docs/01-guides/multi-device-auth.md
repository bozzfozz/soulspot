# Multi-Device Authentication Guide

**Category:** User Guide  
**Version:** 1.0  
**Last Updated:** 2025-01  
**Audience:** Users, API Developers

---

## Overview

SoulSpot supports accessing authenticated sessions from multiple devices or browsers. This guide explains authentication mechanisms and cross-device usage.

---

## How Authentication Works

### Token Storage

**✅ All authentication tokens stored server-side!**

- **Spotify OAuth tokens** (access + refresh) persisted in SQLite database
- **Sessions survive server restarts** (no re-authentication needed)
- **Automatic token refresh** (access tokens expire after 1 hour, automatically renewed with refresh token)

### Authentication Methods

**1. Cookie-based (Default)** - Most secure, browser-managed  
**2. Bearer Token** - For API clients, curl, cross-device access

---

## Single Device (Default)

### Initial Setup

**1. Start OAuth Flow**
```bash
# Visit in browser
http://localhost:8000/api/auth/authorize
```

**2. Authorize with Spotify**
- Redirected to Spotify authorization page
- Grant permissions to SoulSpot
- Automatically redirected back with tokens stored

**3. Session Cookie Set**
- Browser receives `session_id` cookie (HttpOnly, Secure)
- All subsequent requests use cookie for authentication
- No manual token management!

### Token Refresh (Automatic)

- Spotify access tokens expire after **1 hour**
- SoulSpot auto-refreshes tokens when needed
- **No human interaction required** (completely transparent)
- Refresh tokens don't expire (unless revoked by Spotify)

---

## Multi-Device Access

### Use Cases

- Access from work laptop + home desktop
- Use API clients (curl, Postman, scripts) alongside web UI
- Share access with trusted household members (same Spotify account)

### How Multi-Device Works

SoulSpot stores tokens **server-side**. Once authenticated on one device:

1. Spotify tokens stored in database
2. Same account accessible from any device on local network
3. Simply login again on new device (server maintains Spotify connection)

### Accessing from Another Device

**On any device in local network:**

1. Open browser → Navigate to SoulSpot (e.g., `http://192.168.1.100:8000`)
2. Start OAuth flow via `/api/auth/authorize`
3. Login with Spotify credentials
4. Now authenticated on this device!

**Note:** Each device gets own session, but all share same server-side Spotify tokens (no re-authorization needed - just login)

---

### Using Bearer Tokens for API Access

For API clients, scripts, automation:

**Example: List playlists with curl**
```bash
# Get session ID from browser (DevTools → Application → Cookies → session_id)
SESSION_ID="your-session-id-from-browser"

curl -H "Authorization: Bearer $SESSION_ID" \
     http://localhost:8000/api/playlists
```

**Example: Python script**
```python
import os
import requests

# Get session ID from environment
session_id = os.environ.get("SOULSPOT_SESSION_ID")
headers = {"Authorization": f"Bearer {session_id}"}

# Get playlists
response = requests.get(
    "http://localhost:8000/api/playlists",
    headers=headers
)
print(response.json())

# Sync playlist
response = requests.post(
    "http://localhost:8000/api/playlists/sync",
    headers=headers,
    json={"spotify_uri": "spotify:playlist:..."}
)
```

**Example: JavaScript fetch**
```javascript
const sessionId = "your-session-id-from-browser";

fetch("http://localhost:8000/api/playlists", {
  headers: {
    "Authorization": `Bearer ${sessionId}`
  }
})
.then(response => response.json())
.then(data => console.log(data));
```

---

## Security Best Practices

### ⚠️ Session ID is Sensitive Credential

**Session ID grants access to your SoulSpot session!**

### DO ✅

- **Use HTTPS in production** - Never send session IDs over unencrypted connections
- **Keep session IDs secret** - Treat them like passwords
- **Revoke compromised sessions** - Use `/api/auth/logout` if leaked
- **Use cookie-based auth when possible** - More secure than bearer tokens (HttpOnly protection)

### DON'T ❌

- **Don't commit to git** - Add `.env` to `.gitignore`
- **Don't share publicly** - Slack, Discord, GitHub issues, etc.
- **Don't hardcode in scripts** - Use environment variables

### Recommended Practices

**1. Use environment variables for scripts**
```bash
# .env file (never commit!)
SOULSPOT_SESSION_ID=your-session-id-here
```

```python
import os
session_id = os.environ["SOULSPOT_SESSION_ID"]
```

**2. Rotate sessions periodically**
```bash
# Logout old session
curl -X POST http://localhost:8000/api/auth/logout

# Re-authenticate
# Visit http://localhost:8000/api/auth/authorize
```

**3. Monitor session activity**
```bash
# Check current session info
curl http://localhost:8000/api/auth/session
```

---

## API Usage Examples

### Check Authentication Status

```bash
# With cookie (browser)
curl -b cookies.txt http://localhost:8000/api/auth/session

# With bearer token
curl -H "Authorization: Bearer YOUR_SESSION_ID" \
     http://localhost:8000/api/auth/session
```

**Response:**
```json
{
  "session_id": "abc123...",
  "has_access_token": true,
  "has_refresh_token": true,
  "token_expired": false,
  "created_at": "2025-11-24T10:00:00Z",
  "last_accessed_at": "2025-11-24T12:30:00Z"
}
```

### Manual Token Refresh

Normally automatic, but can trigger manually:

```bash
curl -X POST \
  -H "Authorization: Bearer YOUR_SESSION_ID" \
  http://localhost:8000/api/auth/refresh
```

### Logout (Revoke Session)

```bash
curl -X POST \
  -H "Authorization: Bearer YOUR_SESSION_ID" \
  http://localhost:8000/api/auth/logout
```

---

## Troubleshooting

### "No session found" Error

**Cause:** Session ID missing or incorrect

**Solutions:**
1. Check cookie set correctly (DevTools → Application → Cookies)
2. Verify Authorization header format: `Authorization: Bearer <session_id>`
3. Re-authenticate via `/api/auth/authorize`

### "Invalid or expired session" Error

**Cause:** Session expired due to inactivity or server restart (if not using DatabaseSessionStore)

**Solutions:**
1. Check session expiry: `curl http://localhost:8000/api/auth/session`
2. Re-authenticate via `/api/auth/authorize`

### Token Refresh Fails

**Cause:** Refresh token invalid or revoked by Spotify

**Solutions:**
1. Check Spotify app permissions: [Spotify Account Apps](https://www.spotify.com/account/apps/)
2. Revoke and re-grant access to SoulSpot
3. Full re-authentication required (cannot recover from invalid refresh token)

### Session Doesn't Work on Device B

**Checklist:**
- ✅ Both devices can reach SoulSpot server (network/firewall)
- ✅ Cookie domain matches (use IP/hostname consistently)
- ✅ Cookie path is `/`
- ✅ For API clients: Authorization header format correct

### Bearer Token Works but Cookie Doesn't

**Cause:** Cookie security settings mismatch

**Solutions:**
1. Check `API_SECURE_COOKIES` in `.env`
   - `false` for HTTP (local development)
   - `true` for HTTPS (production)
2. Ensure browser and server domain match
3. Check SameSite policy allows cookies

---

## Production Deployment

### Required Settings

```bash
# .env (production)
API_SECURE_COOKIES=true  # Require HTTPS
API_SESSION_MAX_AGE=3600  # Session lifetime (seconds)
```

### HTTPS Setup

**Session IDs MUST use HTTPS in production!**

Use reverse proxy (nginx, Traefik, Caddy) for SSL termination:

```nginx
# nginx example
server {
    listen 443 ssl;
    server_name soulspot.example.com;

    ssl_certificate /path/to/cert.pem;
    ssl_certificate_key /path/to/key.pem;

    location / {
        proxy_pass http://localhost:8000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-Proto https;
    }
}
```

---

## FAQs

**Q: Re-authenticate after server restarts?**  
A: **No!** Sessions persisted in SQLite, survive restarts.

**Q: Access from phone with same account?**  
A: **Yes!** Open SoulSpot in mobile browser and authenticate.

**Q: How long do sessions last?**  
A: Active as long as used. Default timeout = 1 hour **inactivity**. Each request refreshes timeout.

**Q: Multiple people use same account?**  
A: **Technically yes**, but **not recommended**. Each person should authenticate separately for security/audit trails.

**Q: Someone stole my session ID?**  
A: They can access Spotify via SoulSpot. **Immediately logout** to revoke:
```bash
curl -X POST -H "Authorization: Bearer STOLEN_SESSION_ID" \
     http://localhost:8000/api/auth/logout
```

**Q: Multiple active sessions?**  
A: **Yes!** Each device/browser can have own session.

---

## Summary

| Feature | Cookie-based Auth | Bearer Token Auth |
|---------|------------------|-------------------|
| **Security** | ✅ High (HttpOnly, Secure) | ⚠️ Medium (manual handling) |
| **Ease of Use** | ✅ Automatic (browser) | ⚠️ Manual header management |
| **Multi-device** | ✅ Each device authenticates | ✅ Native support |
| **API Clients** | ❌ Not ideal | ✅ Perfect fit |
| **Recommended For** | Web UI, browsers | CLI, scripts, automation |

**Best Practice:** Cookie-based auth in browsers, bearer tokens for API clients/automation.

---

## Related Documentation

- [Setup Guide](./setup-guide.md) - Initial configuration
- [Spotify Auth Troubleshooting](./spotify-auth-troubleshooting.md) - OAuth issues
- [User Guide](./user-guide.md) - Feature documentation

---

**Version:** 1.0  
**Last Updated:** 2025-01
