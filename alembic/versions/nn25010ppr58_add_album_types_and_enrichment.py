...
def upgrade() -> None:
    ...
    with op.batch_alter_table("soulspot_albums") as batch_op:
        ...
        batch_op.add_column(sa.Column("album_artist", sa.String(255), nullable=True))

        # primary_type: Main album category - default is 'album'
        # Values: 'album', 'ep', 'single', 'broadcast', 'other'
        batch_op.add_column(
            sa.Column(
                "primary_type", sa.String(20), nullable=False, server_default="album"
            )
        )
        ...
        batch_op.add_column(
            sa.Column("secondary_types", sa.JSON(), nullable=False, server_default="[]")
        )
    ...
    op.create_table(
        ...
        sa.Column(
            "entity_type", sa.String(20), nullable=False, comment="'artist' or 'album'"
        ),
        sa.Column(
            "entity_id",
            sa.String(36),
    ...
    if various_artist_ids:
        ...
        print(
            f"Migrated {len(various_artist_ids)} Various Artists albums to compilation type"
        )
