...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "check_frequency_hours", sa.Integer(), nullable=False, server_default="24"
        ),
        sa.Column("auto_download", sa.Boolean(), nullable=False, server_default="1"),
        sa.Column(
            "quality_profile", sa.String(20), nullable=False, server_default="high"
        ),
        sa.Column("last_checked_at", sa.DateTime(), nullable=True),
        sa.Column("last_release_date", sa.DateTime(), nullable=True),
        sa.Column(
            "total_releases_found", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column(
            "total_downloads_triggered",
            sa.Integer(),
            nullable=False,
            server_default="0",
        ),
    ...
    op.create_index(
        "ix_artist_watchlists_artist_id", "artist_watchlists", ["artist_id"]
    )
    op.create_index("ix_artist_watchlists_status", "artist_watchlists", ["status"])
    op.create_index(
        "ix_artist_watchlists_last_checked", "artist_watchlists", ["last_checked_at"]
    )
    ...
    op.create_index(
        "ix_filter_rules_type_enabled", "filter_rules", ["filter_type", "enabled"]
    )
    ...
    op.create_index(
        "ix_quality_upgrade_candidates_track_id",
        "quality_upgrade_candidates",
        ["track_id"],
    )
    op.create_index(
        "ix_quality_upgrade_candidates_processed",
        "quality_upgrade_candidates",
        ["processed"],
    )
    op.create_index(
        "ix_quality_upgrade_candidates_improvement_score",
        "quality_upgrade_candidates",
        ["improvement_score"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_quality_upgrade_candidates_improvement_score", "quality_upgrade_candidates"
    )
    op.drop_index(
        "ix_quality_upgrade_candidates_processed", "quality_upgrade_candidates"
    )
    op.drop_index(
        "ix_quality_upgrade_candidates_track_id", "quality_upgrade_candidates"
    )
