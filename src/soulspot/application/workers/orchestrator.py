...
class WorkerOrchestrator:
    ...
    async def start_all(self) -> bool:
        ...
        for priority in sorted(priority_groups.keys()):
            ...
            for info in group:
                ...
                unmet_deps = [
                    ...
                    if dep not in self._workers or self._workers[dep].state != WorkerState.RUNNING
                ]
                if unmet_deps:
                    if info.required:
                        logger.error(
            ...
            if len(ready_workers) > 1:
                logger.info(f"  🔀 Starting {len(ready_workers)} workers in parallel (priority={priority})")
            
            async def start_single_worker(info: WorkerInfo) -> tuple[WorkerInfo, Exception | None]:
            ...
            for info, error in results:
                if error is None:
                    ...
                    if info.required:
                        ...
                        logger.warning(f"  ⚠️  {info.name}: Failed to start (non-required) - {error}")
        ...
        logger.info("=" * 60)
        if success:
            logger.info(f"✅ WORKER ORCHESTRATOR - {started_count} workers started successfully")
        else:
            logger.error(f"❌ WORKER ORCHESTRATOR - Startup failed ({started_count} started)")
        logger.info("=" * 60)
    ...
    async def stop_all(self) -> None:
        ...
        for priority in sorted(priority_groups.keys(), reverse=True):
            ...
            if len(group) > 1:
                logger.info(f"  🔀 Stopping {len(group)} workers in parallel (priority={priority})")
            
            # Stop all workers in this priority group in parallel!
            results = await asyncio.gather(
                *[stop_single_worker(w) for w in group],
                return_exceptions=False
            )
    ...
    async def restart_worker(self, name: str, reason: str = "manual") -> bool:
        ...
        try:
            ...
            info.error = None
            logger.info(f"  ✅ {name}: Restarted successfully (restart #{info.restart_count})")
            return True
                    ...
                    if info.task.done() and info.state == WorkerState.RUNNING:
                        ...
                        if exc:
                            ...
                            if info.restart_count < self.auto_recovery_max_restarts:
                                ...
                                await self.restart_worker(name, f"crash_recovery (attempt {info.restart_count + 1})")
                            else:
                                logger.error(
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "auto_recovery_monitor_running": self._monitor_task is not None and not self._monitor_task.done(),
            "by_state": by_state,
