...
class CompilationAnalyzerService:
    ...
    async def analyze_album(self, album_id: str) -> AlbumAnalysisResult | None:
        ...
        was_compilation = (
            SecondaryAlbumType.COMPILATION.value in current_secondary_types
        )
        ...
        detection = detect_compilation(
            ...
            explicit_flag=was_compilation
            if was_compilation
            else None,  # Preserve prior detection
        )
        ...
        if changed:
            ...
            if (
                detection.is_compilation
                and SecondaryAlbumType.COMPILATION.value not in new_secondary_types
            ):
                new_secondary_types.append(SecondaryAlbumType.COMPILATION.value)
            elif (
                not detection.is_compilation
                and SecondaryAlbumType.COMPILATION.value in new_secondary_types
            ):
        ...
        # Build result
        unique_artists = len({a.lower().strip() for a in track_artists if a})
    ...
    async def analyze_all_albums(
        ...
        stmt = select(AlbumModel.id).join(
            track_count_subq, AlbumModel.id == track_count_subq.c.album_id
        )
        ...
        if only_undetected:
            ...
            stmt = stmt.where(
                ~AlbumModel.secondary_types.contains(
                    [SecondaryAlbumType.COMPILATION.value]
                )
            )
    ...
    async def verify_borderline_albums(
        ...
        for album_id, album_title, diversity in borderline_albums:
            try:
                ...
                logger.warning(
                    f"MusicBrainz verification failed for '{album_title}': {e}"
                )
                results.append(
                    {
                        "album_id": album_id,
    ...
    async def set_compilation_status(
        ...
        if is_compilation == was_compilation:
            logger.debug(
                f"Album '{album.title}' already has compilation={is_compilation}"
            )
